...
from aiogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton # Добавлен импорт кнопок
...
async def analyze_chart(message: Message):
    ...
    try:
        ...
        # --- ОБНОВЛЕННЫЙ ПРОМПТ С УКАЗАНИЕМ ЭКСПИРАЦИИ (2 МИНУТЫ) ---
        prompt = [
            "Ты ультра-фокусированный AI-аналитик по техническому анализу. Твоя задача — предоставить ЧЕТКИЙ ТОРГОВЫЙ СИГНАЛ в виде цифр, основываясь на анализе графика. Обязательно определи и укажи валютную пару/актив. Используй **Smart Money Concepts (SMC)** для определения точек входа.\n\n"
            "**СТРУКТУРА ОТВЕТА (ОЧЕНЬ СТРОГО — ТОЛЬКО КЛЮЧЕВЫЕ ПУНКТЫ):**\n"
            "1. **ПАРА:** Укажи валютную пару или актив (например, EURUSD, BTC/USD).\n"
            "2. **СИГНАЛ:** [ПОКУПКА 🟢 ⬆️] или [ПРОДАЖА 🔴 ⬇️]\n"
            "3. **ВХОД (ENTRY):** Укажи конкретную цену входа.\n"
            "4. **ДЛИТЕЛЬНОСТЬ:** **2 минуты** (Строго, так как анализ ведется на младшем таймфрейме).\n" # ИЗМЕНЕНИЕ ЗДЕСЬ
            "5. **АНАЛИЗ:** Кратко (1-2 слова) укажи причину движения (например: 'реакция на OB', 'заполнение FVG', 'пробой BOS').\n"
            "6. **ФИЛЬТР:** Кратко укажи дополнительное подтверждение (Тренд/Импульс).\n"
            "7. **БАЛАНС:** Комментарий по балансу (если виден).\n"
