...
import logging
import os
import re # Новый импорт для поиска цифр в тексте
...
async def handle_text_message(message: Message):
    ...
    if "торговать" in user_text or "рассчитать" in user_text or "сумм" in user_text or "баланс" in user_text:
        ...
        if numbers:
            try:
                balance = float(numbers[-1]) # Берем последнее число как сумму баланса
                risk_percent = 0.01
                ...
                pass # Продолжаем к общему промпту, если цифры не удалось обработать
    ...
    try:
        ...
        prompt_text = (
            "Ты узкоспециализированный AI-аналитик по Smart Money Concepts (SWMC) и техническому анализу. "
            "Отвечай только по теме трейдинга, анализа, расчетов рисков и SWMC. Ответ должен быть максимально "
            "кратким и по существу. "
            "Если вопрос НЕ связан с трейдингом, финансовым анализом или SWMC, "
            "ответь СТРОГО: 'Я аналитик на ИИ, ваш личный помощник. Моя специализация — технический анализ и .' "
            f"Запрос: {user_text}"
        )
...
async def analyze_chart(message: Message):
    ...
    try:
        ...
        # ФИНАЛЬНЫЙ ПРОМПТ ДЛЯ МАКСИМАЛЬНО ЧЕТКОГО СИГНАЛА БЕЗ УПОМИНАНИЯ SMC В ОТВЕТЕ
        prompt = [
            ...
            "4. **АНАЛИЗ:** Кратко (1-2 слова) укажи причину движения (например, 'реакция на уровень' или 'пробой тренда').\n"
