...
# --- ОБРАБОТЧИК ДЛЯ АНАЛИЗА ГРАФИКОВ (КОМПЛЕКСНЫЕ СИГНАЛЫ) ---
@dp.message(F.photo)
async def analyze_chart(message: Message):
    ...
    status_msg = await message.answer("🚨 Выполняю поиск точек входа по SMC + Фильтры...")
    ...
    try:
        ...
        # НОВЫЙ ПРОМПТ ДЛЯ УВЕЛИЧЕНИЯ ПРОХОДИМОСТИ
        prompt = [
            "Ты ультра-фокусированный AI-аналитик по Smart Money Concepts (SMC). Твоя задача — предоставить ЧЕТКИЙ ТОРГОВЫЙ СИГНАЛ, используя SMC, и **подтвердить его одним из трех дополнительных фильтров (Тренд/Импульс/Настроение рынка)** для увеличения проходимости.\n\n"
            "**СТРУКТУРА ОТВЕТА (ОЧЕНЬ СТРОГО — ТОЛЬКО ЦИФРЫ И КОРОТКИЙ АНАЛИЗ):**\n"
            "1. **КЛЮЧЕВОЕ НАПРАВЛЕНИЕ (2 минуты):** ПОКУПКА (Buy) 🟢 или ПРОДАЖА (Sell) 🔴.\n"
            "2. **ТОЧКА ВХОДА (ENTRY):** Укажи конкретную цену входа.\n"
            "3. **СТОП-ЛОСС (SL):** Укажи конкретную цену стоп-лосса.\n"
            "4. **ТЭЙК-ПРОФИТ (TP):** Укажи конкретную цену для первой цели.\n"
            "5. **ПОДТВЕРЖДЕНИЕ SMC:** Кратко укажи элемент SMC (например, Order Block на 1.24900).\n"
            "6. **ФИЛЬТР:** Кратко укажи, какой дополнительный фильтр подтверждает сделку (например, 'Тренд восходящий' или 'Сильный импульс на покупку').\n"
            "**ОБЯЗАТЕЛЬНОЕ ПРЕДУПРЕЖДЕНИЕ:** Это анализ изображения. Это не финансовый совет и не гарантия прибыли.",
            image 
        ]
